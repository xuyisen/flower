...
def aggregate_inplace(results: list[tuple[ClientProxy, FitRes]]) -> NDArrays:
    ...
    scaling_factors: np.ndarray[Any, Any] = np.asarray(
...
def aggregate_krum(
    ...
    if to_keep > 0:
        ...
        best_results = [results[i] for i in best_indices]  # type: ignore[index]
...
def aggregate_qffl(
    ...
    for i in range(len(deltas[0])):
        ...
        updates.append(tmp)
    new_parameters: NDArrays = [(u - v) * 1.0 for u, v in zip(parameters, updates)]
...
def _compute_distances(weights: list[NDArrays]) -> NDArray:
    ...
    distance_matrix: np.ndarray[Any, Any] = np.zeros((len(weights), len(weights)))
...
def _aggregate_n_closest_weights(
    ...
    for layer_id, layer_weights in enumerate(reference_weights):
        ...
        # closest distances
        beta_closest_weights: np.ndarray[Any, Any] = np.take_along_axis(
